...
const config = {
  libraryName: 'ngx-awesome-table',
  unscopedLibraryName: 'ngx-awesome-table',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ngx-table/ --deploy-url /ngx-table/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ngx-awesome-table-demo:server`, { cwd: `${config.demoDir}` }))
